...
# 传参版本
#!/bin/bash
./ab-test-tool.sh -P $1 -H $2 -K $3 -U $4
```
...
# token
token=''
ARGS=`getopt -a -o N:C:R:S:I:X:J:U:T:P:H:K:h -l count:,client:,round:,sleeptime:,min:,max:,step:,runtime:,postfile:,header:,token:,help -- "$@"`
[ $? -ne 0 ] && usage
eval set -- "${ARGS}" 
while true 
do
