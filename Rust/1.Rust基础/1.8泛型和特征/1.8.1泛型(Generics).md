...
该泛型函数的作用是从列表中找出最大的值，其中列表中的元素类型为 T。首先 `largest<T>` 对泛型参数 `T` 进行了声明，然后才在函数参数中进行使用该泛型参数 `list: &[T]` （还记得 `&[T]` 类型吧？这是[数组切片](../1.4复合类型/1.4.4数组##数组切片)）。
...
在[数组](../1.4复合类型/1.4.4数组)那节，有提到过很重要的一点：`[i32; 2]` 和 `[i32; 3]` 是不同的数组类型，比如下面的代码：
...
>### ***来自Bing的代码解读：***
>
>这段代码的主要部分是定义了一个泛型函数`something`，它接受一个泛型参数`T`和一个`T`类型的值`val`。函数的签名中，使用了`where`子句来给泛型参数`T`添加一个约束，即`Assert<{ core::mem::size_of::<T>() < 768 }>: IsTrue`。这个约束的含义是，`T`类型的大小必须小于768字节，否则编译器会报错。这里用到了两个自定义的类型：`Assert`和`IsTrue`，它们的定义在代码的后半部分。
>
 `Assert`是一个泛型枚举类型，它有一个const泛型参数`CHECK`，它的类型是`bool`，表示一个布尔值。这个枚举类型没有任何变体，它只是用来包裹一个常量值。`IsTrue`是一个自定义的trait，它没有任何方法，它只是用来标记一个类型是否为真。这个trait只为`Assert<true>`类型实现了，表示只有当`Assert`的const泛型参数为`true`时，才满足`IsTrue`这个trait。这样，我们就可以用`Assert`和`IsTrue`来实现一个编译期的断言，即如果`Assert`的const泛型参数为`false`，那么编译器就会报错，因为`Assert<false>`没有实现`IsTrue`这个trait。
 >
 回到`something`函数的签名，我们可以看到，`Assert`的const泛型参数是一个const表达式，即`core::mem::size_of::<T>() < 768`。这个表达式的含义是，计算`T`类型的大小然后判断它是否小于768，返回一个布尔值。这里用到了`core::mem::size_of`这个函数，它是Rust标准库中的一个函数，它可以返回一个类型的大小，单位是字节。这个函数是一个const函数，即它可以在编译期被求值，所以它可以用在const表达式中。这样，我们就可以用这个const表达式来约束`T`类型的大小，如果`T`类型的大小大于等于768，那么这个const表达式就会返回`false`，导致`Assert<false>`没有实现`IsTrue`这个trait，从而编译器报错。
## 泛型的性能
