...
![image-20231224182755869](assets/image-20231224182755869.png)

## 函数要点

- 函数名和变量名使用[蛇形命名法(snake case)](../../4.最佳实践/4.2命名规范)，例如 `fn add_two() -> {}`
...
函数的返回值就是函数体最后一条表达式的返回值，也可以使用 `return` 提前返回，下面的函数使用最后一条表达式来返回一个值：
...
> 只有表达式能返回值，而 `;` 结尾的是语句，在 Rust 中，一定要严格区分[语句和表达式](1.4语句和表达式)的区别，这个在其它语言中往往是被忽视的点。
